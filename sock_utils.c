...
void socketInit () {
    // Создание сокета, инициализация адреса и соединения
...
void releaseRequest () {
    // Формирование соединения и отправка запроса
    if (!bExternalIP) {
    ...
    if (strcmp (json_object_get_string (certainJsonObj), "success")) {
        puts ("[\033[1;31m-\033[0m] API error: Request status is not \"success\"");
        exit (7);
    }
    ...
    if (!json_object_is_type (certainJsonObj, json_type_null) && json_object_get_string_len (certainJsonObj)) {
        ...
        printf ("[\033[1;31m-\033[0m] %s: \033[1;31mОшибка\033[0m\n", paramStrs [0][0]);
    } if (!only4) {
